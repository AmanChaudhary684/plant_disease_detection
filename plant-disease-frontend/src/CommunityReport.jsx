...
export function CommunityReportButton({ result, lang }) {
  ...
  // ✅ FIX: Show for ALL results — both diseased AND healthy plants
  if (!result) return null;

  const isHealthy = result.diagnosis?.is_healthy;
  ...
  return (
    <>
      <button
        ...
        {isHealthy ? "✅" : "📍"}{" "}
        {lang === "hi"
          ? (isHealthy ? "स्वस्थ पौधा रिपोर्ट करें" : "समुदाय को रिपोर्ट करें")
          : (isHealthy ? "Report Healthy Plant" : "Report to Community")}
      </button>
      {open && (
...
function CommunityReportModal({ disease, isHealthy, lang, onClose }) {
  ...
  const detectLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(async pos => {
      try {
        ...
        const matched = INDIA_STATES.find(
          s => raw.toLowerCase().includes(s.toLowerCase()) ||
               s.toLowerCase().includes(raw.toLowerCase())
        );
        if (matched) setState(matched);
        if (d.address?.city || d.address?.town)
          setCity(d.address.city || d.address.town || "");
      } catch {}
    });
  };
  ...
  const MOD = {
    background: "linear-gradient(160deg,#071a0b,#050e07)",
    border: `1px solid ${isHealthy ? "rgba(74,222,128,0.3)" : "rgba(251,146,60,0.3)"}`,
    ...
    <div style={OV} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={MOD}>
        ...
        <div style={{
          ...
          borderBottom: `1px solid ${isHealthy ? "rgba(74,222,128,0.15)" : "rgba(251,146,60,0.15)"}`,
        }}>
          <div>
            <div style={{ fontSize: 18, fontWeight: 700, color: "#f0fdf4", marginBottom: 4 }}>
              {isHealthy ? "✅" : "📍"}{" "}
              {lang === "hi"
                ? (isHealthy ? "स्वस्थ पौधा रिपोर्ट करें" : "रोग की रिपोर्ट करें")
                : (isHealthy ? "Report Healthy Plant" : "Report Disease Outbreak")}
            </div>
            <div style={{ fontSize: 12, color: "#6ee7b7" }}>
              {lang === "hi"
                ? "किसानों को सतर्क करने के लिए रिपोर्ट करें"
                : "Help farmers by reporting this observation in your area"}
            </div>
          </div>
          <button
            onClick={onClose}
            style={{ background: "transparent", border: "none", color: "#6b7280", fontSize: 20, cursor: "pointer", lineHeight: 1 }}>
            ×
          </button>
        </div>
        ...
        {!submitted ? (
          <div style={{ padding: 20 }}>
            ...
            <div style={{
              ...
              background: isHealthy ? "rgba(74,222,128,0.1)" : "rgba(239,68,68,0.1)",
              border: `1px solid ${isHealthy ? "rgba(74,222,128,0.25)" : "rgba(239,68,68,0.25)"}`,
              borderRadius: 12, padding: "10px 14px", marginBottom: 16,
            }}>
              <span style={{ fontSize: 16 }}>{isHealthy ? "🌿" : "🔬"}</span>
              <div>
                <div style={{ fontSize: 11, color: "#6ee7b7", marginBottom: 2 }}>
                  {lang === "hi" ? "पहचाना गया:" : "Identified:"}
                </div>
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={{ display: "block", fontSize: 12, color: "#86efac", fontWeight: 600, marginBottom: 6 }}>
                🗺️ {lang === "hi" ? "राज्य चुनें *" : "Select Your State *"}
              </label>
              <div style={{ display: "flex", gap: 8 }}>
                <select
                  ...
                  style={{
                    ...
                    padding: "10px 12px", color: "#f0fdf4", fontSize: 14,
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 10, padding: "0 14px", fontSize: 18, cursor: "pointer",
                  }}>
                  📍
                </button>
              </div>
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={{ display: "block", fontSize: 12, color: "#86efac", fontWeight: 600, marginBottom: 6 }}>
                🏙️ {lang === "hi" ? "शहर (वैकल्पिक)" : "City (optional)"}
              </label>
              <input
                value={city}
                onChange={e => setCity(e.target.value)}
                placeholder={lang === "hi" ? "जैसे दिल्ली, पुणे..." : "e.g. Delhi, Pune..."}
                style={{
            ...
            <div style={{
              ...
              <span>ℹ️</span>
              <span style={{ fontSize: 12, color: "#a7f3d0", lineHeight: 1.5 }}>
                {lang === "hi"
                  ? "आपकी रिपोर्ट भारत के रोग प्रकोप मानचित्र पर दिखेगी।"
                  : "Your report will appear on the India disease outbreak map and alert nearby farmers."}
              </span>
            </div>
            ...
            {error && (
              <div style={{
                ...
                borderRadius: 10, padding: "10px 12px", color: "#f87171", fontSize: 13, marginBottom: 12,
              }}>
                ⚠️ {error}
              </div>
            )}
            ...
            {/* Submit button */}
            <button
              onClick={() => submitReport(disease, state, city || null)}
              ...
              {submitting
                ? "Submitting..."
                : lang === "hi" ? "✅ रिपोर्ट सबमिट करें" : "✅ Submit Report"}
            </button>
          </div>
        ) : (
          <div style={{ padding: "32px 20px", textAlign: "center" }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: "#4ade80", marginBottom: 8 }}>
              {lang === "hi" ? "रिपोर्ट सबमिट हो गई!" : "Report Submitted!"}
            </div>
            <div style={{ fontSize: 14, color: "#d1fae5", marginBottom: 8, lineHeight: 1.6 }}>
              {lang === "hi"
                ? `${state} के किसानों को सतर्क किया गया है।`
                : `Farmers in ${state} have been alerted about ${displayDisease}.`}
            </div>
            <div style={{ fontSize: 12, color: "#6ee7b7", marginBottom: 20 }}>
              {lang === "hi" ? "यह रोग प्रकोप मानचित्र पर दिख रहा है।" : "Now visible on the outbreak map."}
            </div>
            <button
              onClick={onClose}
              style={{
...
export function LiveFeedTicker({ lang }) {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/community/recent?limit=15`)
      .then(r => r.json())
      ...
      <div style={{ display: "flex", gap: 4 }}>
        {reports.slice(0, 5).map((_, i) => (
          <div key={i} style={{
            ...
            background: i === idx % 5 ? "#fb923c" : "rgba(255,255,255,0.2)",
